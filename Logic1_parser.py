"""
Logic1_parser.py — CAESAR II .CII file parser for Logic 1.
Ported from P1 parser.py. Only change: logger import uses Logic1_logger.
"""
import pydantic
import logging
...
logger = get_logger("Logic1_parser")
...
class CIIParser:
    def __init__(self, settings: ParserSettings):
        ...
        logger.info(f"Initialized Logic1 CIIParser with N1={self.n1}")
    ...
    def _parse_version(self, ctx: ParseContext):
        ...
        logger.info(f"Parsed version: {self.parsed_data['version']['GVERSION']}")
    ...
    def _parse_aux_data(self, ctx: ParseContext):
        ...
        while ctx.has_more() and ctx.peek().startswith("#$ ") and ctx.peek().strip() not in ["#$ MISCEL_1", "#$ UNITS", "#$ COORDS"]:
